    ...
    return (
        <div className="bg-gray-50 min-h-screen flex flex-col">
            ...
            <div className="container mx-auto px-4 py-8 sm:py-12 text-black flex-1">
                <h1 className="text-3xl sm:text-4xl font-bold text-center mb-8 sm:mb-12">Product Enquiry</h1>                {enquiryItems.length === 0 ? (
                    <EmptyState
                        icon={
                        ...
                        <div className="xl:col-span-2 order-2 xl:order-1">
                            <Card className="card-no-hover">
                                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
                                    <h2 className="text-xl sm:text-2xl font-bold">Selected Products ({getTotalItems()} items)</h2>
                                    <Button
                                        onClick={clearEnquiry}
                                        variant="danger"
                                        size="sm"
                                        className="transform-none hover:transform-none hover:shadow-none w-full sm:w-auto"
                                    >
                                        Clear All
                                    </Button>
                                </div>
                                ...
                                <div className="space-y-4">
                                    {enquiryItems.map((item, index) => (
                                        <div key={item.id} className="flex flex-col sm:flex-row sm:items-center border-b pb-4 gap-4">
                                            {item.image ? (
                                                <img src={item.image} alt={item.name} className="w-full sm:w-20 h-48 sm:h-20 object-cover rounded" />
                                            ) : (
                                                <div className="w-full sm:w-20 h-48 sm:h-20 bg-gray-200 rounded flex items-center justify-center">
                                                    <span className="text-gray-500 text-xs">No Image</span>
                                                </div>
                                            )}

                                            <div className="flex-1">
                                                <h3 className="font-semibold text-gray-900 text-lg sm:text-base">{item.name}</h3>
